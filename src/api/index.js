...
export const reqSearchShop = (geohash, keywords) => ajax(`/search_shops/${geohash, keywords}`);
// 5、获取一次性验证码](#5获取一次性验证码)
export const reqCaptcha = () => ajax(`/captcha`);
// 6、用户名密码登陆](#6用户名密码登陆)
export const reqLogin = (name, pwd, captcha) => ajax(`/login_pwd`, {name, pwd, captcha});
// 7、发送短信验证码](#7发送短信验证码)
export const reqAuthCode = (phone) => ajax(`/sendcode${phone}`);
// 8、手机号验证码登陆](#8手机号验证码登陆)
export const reqLoginWithAuthCode = (phone, code) => ajax(`/login_sms`, {phone, code});
